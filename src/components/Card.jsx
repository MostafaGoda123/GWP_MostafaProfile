...
const Card = ({project}) => {
   ...
   return (
      <motion.div className='bg-white rounded-xl border-2 border-neutral-400 shadow-xl p-2 flex flex-col gap-4 items-center justify-center'
      initial={{y:100}}
      whileInView={{y:0}}
      transition={{duration:0.4}}>
         <img src={project.image} alt="..." className="w-full h-60 rounded-xl" />
         <h1 className="font-bold text-2xl">{project.name}</h1>
         <p className="text-2xl text-neutral-600">(<span className='font-semibold mx-1npm start
         '>{project.category}</span>) project</p>
